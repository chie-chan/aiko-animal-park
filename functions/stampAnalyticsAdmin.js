...
export async function onRequest(context) {
  ...
  for (const event of events) {
    ...
    if (day) {
      ...
      if (eventName === "export_zip") day.downloads += 1;
      if (eventName === "prompt_copy") day.promptCopies += 1;
    }
  ...
  const summary = {
    ...
    downloads: events.filter((event) => event.event === "export_zip").length,
    promptCopies: events.filter((event) => event.event === "prompt_copy").length,
